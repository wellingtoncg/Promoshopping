		    ...
		    <div class="popover-inner">
		      <div class="list-block">
		        <ul>
		          ...
		          <li><a href="my-profile.php" class="list-button item-link clickopen">Meu Perfil</a></li>
		          <!--
		          colocar aqui um IF (PHP) pra consultar a tabela 'usuario_loja'
		          -->
		          <li><a href="my-store.php" class="list-button item-link clickopen">Minha loja</a></li>
        ...
        <div class="pages navbar-through toolbar-through">
          ...
          ?>
			...
			<?php
					...
					////
					$sqlPaginacao = mysql_query("SELECT * FROM produtos WHERE status = 1 ORDER BY codigo ASC"); //retorna o total de registros da tabela
					$paginas = intval((mysql_num_rows($sqlPaginacao)-1) / $limite); //faz o cálculo do número de páginas
					////
					
					$sql = "select codigo, date_format(data_postagem, '%d/%m/%Y') as data_postagem, codigo_usuario, codigo_loja, codigo_categoria, nome, preco_original, preco_atual, imagem from produtos WHERE status = " . 1 . " order by codigo desc LIMIT $inicio, $limite";
			...
			<?php
				...
				if(($pagina) > 1) {
    			echo '<li><a href="index.php?pagina=1">&laquo; Primeira</a></li>';
    			$anterior = $pagina - 1;
				...
				for ($i = ($pagina-1)-$links; $i<($paginas + 1); $i++) {
				    if ($i == $pagina -1) { //verifica se não é a página atual
				        ...
				        if($i+1>0 && $i<$pagina+$links) {
				            $pag = $i+1;
				            echo '<li><a href="?pagina='.$pag.'">'.$pag.'</a>&nbsp;</li>';
				        }
				    }
				}
				...
				if ($pagina-1 < $paginas) {
				    $proxima = $pagina+1;
				    echo '<li><a href="?pagina='.$proxima.'">Próxima &rsaquo;</a></li>';
				    $ultima = $paginas + 1;
				    echo '<li>&nbsp;<a href="?pagina='.$ultima.'">Última &raquo;</a></li>';
				}
			?>
				</ul>
			</div>
